...
class DecoderRNN(BaseRNN):
    ...
    def __init__(self, vocab_size, max_len, hidden_size, embedding_size,
        ...
        #                          batch_first=True, bidirectional=bidirectional, dropout=dropout_p)
        self.rnn = self.rnn_cell(64, hidden_size, n_layers,
                                 batch_first=True, bidirectional=bidirectional, dropout=dropout_p)
    ...
    def forward_step(self, input_var, input_part, hidden, encoder_outputs, function):
        ...
        # print("encoder hidden[0]: {}".format(hidden[0].shape))
        # print("encoder hidden[1]: {}".format(hidden[1].shape))
        # print("embedded hidden[0]: {}".format(embedded.shape))
        # print("encoder hidden[0]: {}".format(hidden[0].shape))
        # print("encoder hidden[1]: {}".format(hidden[1].shape))
        output, hidden = self.rnn(embedded, hidden)

        # print("decoder hidden[0]: {}".format(hidden[0].shape))
        # print("decoder hidden[1]: {}".format(hidden[1].shape))
